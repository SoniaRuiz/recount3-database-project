...
TidyDataPiorSQL <- function (recount3.project.IDs,
  ...
  if (replace) {
    ...
    if (intersect(df_not_paired_tidy$junID, df_all_jxn_pairings$novel_junID) %>% length() > 0) {
      stop("ERROR! Some paired junctions have been erroneously identified as non-paired!")
    }
    ...
    if( intersect(df_not_paired_tidy$junID, df_all_jxn_pairings$novel_junID) %>% length() > 0 |
      ...
      stop("ERROR! Some paired junctions have been erroneously identified as never-misspliced!")
    }
