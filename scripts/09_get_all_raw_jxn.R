...
GetAllAnnotatedSplitReads <- function(recount3.project.IDs,
  ...
  if (replace) {
      ...
      if (!is.null(all.clusters)) {
        ...
        all_jxn_qc <- map_df(all.clusters, function(cluster) {
          ...
          if (file.exists(file.path(folder_results_root, "base_data", paste0(project_id, "_", cluster, "_all_split_reads.rds")))) {
            ...
            all_split_reads_details %>% distinct(junID, .keep_all = T) %>% as_tibble() %>% return()
        ...
        if (all_jxn_qc %>% nrow() > 0 ) {
          all_jxn_qc %>% distinct(junID, .keep_all = T) %>% return()
        } else { return(NULL) }
      }
    }
    ...
    saveRDS(object = all_split_reads_details_level2 %>% as_tibble() %>% distinct(junID, .keep_all = T),
        ...
        map_df(all.clusters, function(cluster) {
          ...
          if (file.exists(file.path(folder_results_base, paste0(project_id, "_", cluster, "_samples_used.rds")))) {
            ...
            if (samples %>% length() > 0) {
              ...
              if (!file.exists(file.path(folder_cluster_pairings, paste0(cluster, "_raw_distances_tidy.rds"))) || replace) {
                ...
                df_all <- readRDS(file = file.path(folder_cluster_pairings, paste0(cluster, "_raw_distances_tidy.rds")))
              }
              
              
              if (nrow(df_all) > 0) {
                
                df_all %>% distinct(novel_junID, ref_junID, .keep_all = T) %>% 
  ...
  if (replace) {
    ...
    all_split_reads_ambig <- map_df(recount3.project.IDs, function(project_id) { 
      ...
      map_df(clusters, function(cluster_id) { 
        ...
        if (file.exists(file.path(results_folder_local, paste0(project_id, "_", cluster_id, "_all_split_reads_ambiguous.rds")))) {
          
          ## Load all split reads
          readRDS(file = file.path(results_folder_local, paste0(project_id, "_", cluster_id, "_all_split_reads_ambiguous.rds"))) %>%
            dplyr::select(-any_of(c("in_ref","n_projects","annotated"))) %>%
            unnest(gene_id) %>% 
