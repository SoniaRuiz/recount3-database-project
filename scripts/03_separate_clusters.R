...
SeparateClusters <- function(recount3.project.IDs,
  ...
  if ( data.source == "data_sources/gtex" ) {
    ...
    project_metadata_tidy <- data.frame(sample_id =  project_metadata_tidy$external_id %>% as.character(),
                                        ...
                                        cluster = str_remove_all(project_metadata_tidy$gtex.smtsd, " "),
    ...
    if ( recount3.project.IDs == "SRP151040" ) {
      ...
      project_metadata_tidy <- project_metadata_tidy %>%
        rowwise() %>%
        dplyr::mutate(cluster = paste0(`cell type`, " - " ,`disease state`) %>% str_remove_all(pattern = "\\'") %>% str_remove(" ")) %>%
        ungroup()
      ...
      project_metadata_tidy <- project_metadata_tidy %>%
        ...
        dplyr::mutate(cluster = cluster %>% str_remove(pattern = " ")) %>%
        ungroup() %>%
      ...
      project_metadata_tidy <- project_metadata_tidy %>%
        ...
        mutate(age = age %>% as.integer()) %>%
        as_tibble()
    } else if ( recount3.project.IDs == "SRP103588" ) {
      
      project_metadata_tidy <- project_metadata_tidy %>%
        rowwise() %>%
        dplyr::mutate(cluster = ifelse( test = str_detect(sra.experiment_title, pattern="CTRL"),
                                 yes = "Control",
                                 no = "FamilialAD")) %>%
        ungroup() 
    }
    ...
    project_metadata_tidy <- project_metadata_tidy %>%
      ...
      project_metadata_tidy <- data.frame(sample_id =  project_metadata_tidy$external_id %>% as.character(),
                                          ...
                                          #rin = project_metadata_tidy$gtex.smrin %>% as.double(),
                                          gender = project_metadata_tidy$tcga.gdc_cases.demographic.gender |> as.character(),
                                          ethnicity =  project_metadata_tidy$tcga.gdc_cases.demographic.ethnicity |> as.character(),
                                          race = project_metadata_tidy$tcga.gdc_cases.demographic.race |> as.character(),
                                          
                                          individual_id = project_metadata_tidy$tcga.gdc_cases.submitter_id %>% as.character(),
                                          
                                          cluster = str_remove_all(project_metadata_tidy$tcga.cgc_sample_sample_type |> as.character(), pattern = " "),
                                          tumor = project_metadata_tidy$tcga.gdc_cases.project.name |> as.character(),
                                          site = project_metadata_tidy$tcga.cgc_case_primary_site |> as.character(),
