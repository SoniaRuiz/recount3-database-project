...
dummie_testing <- function() {
  ...
  cluster_id <- "control"
  table_name <- paste0(cluster_id, "_", project_name, "_misspliced")
  split_read_counts <- readRDS(file = paste0(results_folder, "/", project_name, "/base_data/",project_name, "_", cluster_id, "_split_read_counts.rds"))
  ...
  query = paste0("SELECT ref_junID, ref_coordinates FROM 'intron' WHERE ref_junID == ", junID)
...
plot_GO_terms <- function() {
  ...
  introns_increasing_msra <- common_introns_subsample_column %>%
  ...
  ego_MSRA <- clusterProfiler::enrichGO(
    gene          = introns_increasing_msra$gene_name %>% unique,
  ...
  ggplot2::ggsave(paste0(figures_path, "/go_enrichment_msra.png"), width = 180, height = 100, units = "mm", dpi = 300)
